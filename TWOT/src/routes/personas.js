...
router.get(["/Personas", "/Leer"],(req, res)=>{
...
router.get(["/Personas/Leer/:cod"],(req, res)=>{
...
router.post('/personas/agregar',(req,res)=>{
    const objPersona ={
...
//actualizar
router.put('/personas/actualizar/:cod',(req,res)=>{
    const  {cod}= req.params;
    const objPersona ={
...
router.delete('/personas/borrar/:cod',(req,res)=>{
    const{cod}= req.params;
...
router.delete('/Telefonos/borrar/:cod',(req,res)=>{
    const{cod}= req.params;
...
router.get('/Correos/verificar/:email:contra',(req, res,)=>{
     const {email} =  req.params.email;
     const {contra}= req.params.contra;

     const sql=`call PROC_VERIFICAR(${email},${contra},'','', 1)`
...
router.post('/Correos/nuevo',(req,res)=>{
    ...
    const sql = `CALL PROC_CORREOS(${objCorreos.COD_PERSONA}, ${objCorreos.CORREO},${objCorreos.CONTRASEÑA}, 1, '')`
    mysql.query(sql, error=>{
        if(error) throw error;
        res.send('Los datos se insertaron correctamente')
    })
...
const sql = `CALL PROC_CORREOS(${objCorreos.COD_PERSONA}, ${objCorreos.CORREO},,${objCorreos.CONTRASEÑA}, 2, ${cod})`
mysql.query(sql, error=>{
    if(error) throw error;
    res.send('Los datos se actualizaron correctamente')
})
